...
class Solution{
  public:
    ...
    void fillPreorder(Node *root,vector<int> &inorderdata,int &index) {
        if(!root)
        return;
        
        fillPreorder(root->left,inorderdata,index);
        fillPreorder(root->right,inorderdata,index);
    ...
    void convertToMaxHeapUtil(Node* root)
    {
        ...
        int index = 0;
        fillPreorder(root,inorderData,index);
    }    
};
