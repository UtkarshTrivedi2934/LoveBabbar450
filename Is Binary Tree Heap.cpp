...
class Solution {
  public:
    ...
    bool isCBT(struct Node* root, int ind, int total) 
    {
        ...
        bool left = isCBT(root->left, 2*ind+1, total);
        bool right = isCBT(root->right, 2*ind+2, total);
    ...
    bool isHeap(struct Node* tree) {
        ...
        int ind = nodeCount(tree);
        bool CBT = isCBT(tree, 0, ind);
        
        return (CBT and maxOrder(tree));
    }
};
